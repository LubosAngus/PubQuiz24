  ...
  pageTransition: {
    onEnter(el, done) {
      ...
      for (let i = 0; i < bgEls.length; i++) {
        const bgEl = bgEls[i]
        const startX = (100 / bgEls.length) * i
        const endX = (100 / bgEls.length) * (i + 1) + 0.4 / bgEls.length

        tl.fromTo(
          bgEl!,
          {
            translateY: '60%',
            clipPath: `polygon(${startX}% 40%, ${endX}% 40%, ${endX}% 100%, ${startX}% 100%)`,
          },
          {
            translateY: 0,
      ...
      for (let i = 0; i < bgEls.length; i++) {
        ...
        tl.fromTo(
          ...
          },
          {
            opacity: 0.9,
            translateY: '-60%',
            clipPath: `polygon(${startX}% 0%, ${endX}% 0%, ${endX}% 60%, ${startX}% 60%)`,
...
<template>
  <div class="js-TMP">
    <template v-for="i in 10" :key="i">
      <div class="q-absolute-full text-center js-content">
        ...
        <h1
          class="js-text text-8xl font-bold z-10 text-surface-0 text-center q-absolute-full grid place-items-center"
        >
          {{ topic?.name }}
        </h1>
      </div>
